'use strict';
/* secp256k1 — and specifically, public-key recovery.
 *
 * Node's built-in crypto can sign and verify with this curve, but it cannot
 * recover a public key from a signature, and recovery is the whole reason an
 * EVM chain needs the curve at all: a transaction carries no sender field, so
 * the sender IS whatever public key its signature recovers to. Nothing in the
 * platform gives us that, so the field and group arithmetic is here.
 *
 *   y^2 = x^3 + 7  over  F_p,  p = 2^256 - 2^32 - 977
 *
 * Points are held in Jacobian coordinates (x = X/Z^2, y = Y/Z^3) so that
 * addition and doubling cost only multiplications. An affine implementation
 * needs a modular inverse per point operation — hundreds per signature,
 * thousands per block — and is far too slow to validate a block. We pay for
 * exactly one inverse at the end, when converting back to affine.
 *
 ...
 *     message) by HMAC-DRBG rather than trusted to an RNG.
 ...
 *     accepting both would give every transaction two valid encodings and two
 *     hashes. See the note on `verify` for where that rule must NOT apply.
 *
 * What it does NOT have: constant-time execution. BigInt branches and
 * allocates, so timing is data-dependent and this must not be handed a private
 * key an attacker can repeatedly time. Signing here is for tests and tools;
 * production custody is ForgeKeyvault's problem, and validation — the path
 * that actually matters on a node — only ever touches public data.
 */
...
const N_HALF = N >> 1n;                                   // the EIP-2 boundary
 ...
 * paid lazily on first use so requiring this module stays cheap.
 */
function table() {
...
/** Accept a Buffer or a hex string (with or without 0x) of the expected length. */
function asBytes(v, len, what) {
  let b = v;
  if (typeof b === 'string') b = Buffer.from(b.replace(/^0x/i, ''), 'hex');
  if (!Buffer.isBuffer(b)) throw new TypeError(`secp256k1: ${what} must be a Buffer or hex string`);
...
function encodePoint(affine, compressed) {
  if (!affine) throw new Error('secp256k1: cannot encode the point at infinity');
  if (compressed) return Buffer.concat([Buffer.from([2 + Number(affine.y & 1n)]), bigToBuf32(affine.x)]);
  return Buffer.concat([Buffer.from([4]), bigToBuf32(affine.x), bigToBuf32(affine.y)]);
}

// ---- keys ------------------------------------------------------------------

const isValidPrivateKey = priv => {
...
function publicKeyFromPrivate(priv, compressed = false) {
 ...
 */
function rfc6979Nonce(msgHash, priv, accept = () => true) {
  ...
  k = hmac(k, v, Buffer.from([0]), x, bh); v = hmac(k, v);
  k = hmac(k, v, Buffer.from([1]), x, bh); v = hmac(k, v);
  for (;;) {
    v = hmac(k, v);                                 // qlen === hash length, so one pass
 ...
 *
 * It must be passed false by the `ecrecover` precompile: that precompile has
 * never enforced low-s, and Uniswap V2's `permit` — among much else — relies on
 * it not doing so. Enforcing it there would break audited contracts.
 */
function verify(msgHash, sig, pub, { lowS = true } = {}) {
  const h = asBytes(msgHash, 32, 'message hash');
...
/**
 * Recover the signing public key. Returns a 65-byte uncompressed key, or null
 * if no key can have produced this signature — a bad signature is an invalid
 * transaction, not an exception on the validation path.
 *
 *   R  = the point with x = r (+ n if recoveryId says it wrapped) and the
 *        y-parity recoveryId gives
 *   Q  = r^-1 (sR - zG)
 */
function recoverPublicKey(msgHash, sig, opts = {}) {
...
module.exports = {
  P, N, N_HALF, G: { x: GX, y: GY },
  sign, verify, recoverPublicKey, publicKeyFromPrivate,
  isValidPrivateKey, randomPrivateKey,
  decodePoint, encodePoint, toAffine, mulG, mul, jAdd, jDouble,
  rfc6979Nonce, bufToBig, bigToBuf32,
};
