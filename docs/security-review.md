...
| **H10** | **The miner's coinbase key was a plaintext file, and it was the only option.** `src/coinbase.js` wrote the 32-byte key in the clear at mode 600 and read it back with no alternative, so a funded miner's whole balance sat behind one readable file. Measured on the estate 2026-08-09: the mainnet coinbase held **47,421.445463215 EMBER**, accruing ~5.4 per block at ~49 s a block (~400/hour), and the file was bind-mounted **read-write** into the miner container — so container compromise sufficed to read it *or overwrite it*, without a host account and without an escape. | 🟡 `resolveCoinbaseKey` now takes the key from `HEARTH_COINBASE_KEY`, `HEARTH_COINBASE_KEY_FILE`, an encrypted `<data>/coinbase-keystore.json` (scrypt N=2¹⁸ + AES-256-GCM) or the plaintext file, in that order; `HEARTH_COINBASE_ADDRESS` refuses to start rather than mine to an unexpected address; `HEARTH_COINBASE_SOURCE` turns off both the search and key creation; `hearth minerkey seal\|new\|verify\|status` are the operations, and none of them prints a key. Suite: `test/coinbase-source.js`. **Not closed:** a process that signs proofs holds a spendable key in memory whatever it was wrapped in at rest — `verifyPow` recovers the coinbase *from* the proof signature — so the residual is bounded by ROTATING what future blocks pay and sweeping the old balance, not by encryption. [`mining-key-custody.md`](mining-key-custody.md), micro-org#206. |
